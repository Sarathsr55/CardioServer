...
const getAllUsers = async () => {
    try {
        const userResponse = await MongoDB.db.collection(mongoConfig.collections.Patients).aggregate([{$project:{password:0}}]).toArray()
...
const getAllDoctors = async () => {
    try {
        const userResponse = await MongoDB.db.collection(mongoConfig.collections.Doctors).aggregate([{$project:{password:0}}]).toArray()
        if(userResponse?.length>0){
            return userResponse
        }
        else{
...
module.exports = {getUserData,getAllUsers,getUser,getUserById,getAllDoctors}